...
class CqlDelimDeleteTask implements Callable<Long> {
    private String BADPARSE = ".BADPARSE";
    private String BADDELETE = ".BADDELETE";
    ...
    private PrintStream badDeletePrinter = null;
    ...
    public CqlDelimDeleteTask(String inCqlSchema, String inDelimiter,
                            ...
                            int inQueryTimeout, long inMaxDeleteErrors,
        ...
        maxDeleteErrors = inMaxDeleteErrors;
    ...
    private void setup() throws IOException, ParseException, org.json.simple.parser.ParseException {
        ...
        if (null != badDir) {
            ...
            badDeletePrinter = new PrintStream(new BufferedOutputStream(new FileOutputStream(badDir + "/" + readerName + BADDELETE)));
        ...
        delete = cdp.generateDelete();
        statement = session.prepare(delete);
    ...
    private void cleanup(boolean success) throws IOException {
        ...
        if (null != badDeletePrinter) {
            if (format.equalsIgnoreCase("jsonarray"))
                badDeletePrinter.println("]");
            badDeletePrinter.close();
        }
        if (null != logPrinter)
            logPrinter.close();
        if (success) {
    ...
    private int sendDelete(List<Object> elements, String line) {
    ...
    private long execute() throws IOException {
        ...
        if (format.equalsIgnoreCase("delim")
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (null != elements) {
                    int ret = sendDelete(elements, line);
                    if (-2 == ret) {
                        cleanup(false);
                        return -2;
                    }
        ...
        if (null != logPrinter) {
            logPrinter.println("*** DONE: " + readerName + "  number of lines processed: " + lineNumber + " (" + numDeleted + " deleted)");
        }
        System.err.println("*** DONE: " + readerName + "  number of lines processed: " + lineNumber + " (" + numDeleted + " deleted)");
